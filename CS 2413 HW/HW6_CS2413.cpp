...
// 
int BSearch(int key, int* list, int begin, int end) {
	int m = (begin + end) / 2;
	...
	if (begin == end) {
		if (cElem == key) { return begin; } //either begin or end will suffice

		else { return -1; }
	}

	if (cElem == key) { return m; }
	else if (cElem < key) { return BSearch(key, list, m + 1, end); }
	else if (cElem > key) { return BSearch(key, list, begin, end / 2); }
